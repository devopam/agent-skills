...
- **Payments/billing SDKs (Stripe Billing, etc.)** — why excluded: billing is a named open question for whether it's in scope for this skill at all (see stack.md open questions); excluded here pending that decision to avoid scope creep into a large sub-topic on spec.
...
1. Should this baseline include a Python/Django-specific background-job managed alternative (there isn't a strong Python equivalent to Inngest/Trigger.dev surfaced in this research — Celery remains the only well-established Python option), or is that gap acceptable to leave unfilled? This is a genuine coverage-scope call, not something a further fetch resolves.
2. Confirm whether "last reviewed" dates in the eventually-authored skill file should be refreshed on some fixed cadence (e.g., quarterly) given how fast several of these signals move in practice (this pass alone found Inngest's actual license (SSPL) and Avo's actual license (LGPLv3) were both mis-stated by GitHub's own license detector as NOASSERTION, only resolved by reading the LICENSE file directly) — a cadence decision for the maintained skill, not something this research can answer on its own.
3. This baseline recommends ActiveAdmin over Avo when license permissiveness matters (MIT vs. Avo's LGPLv3 open-source tier) but names Avo as the more actively-promoted option in current comparisons. Is a two-option "MIT-first, mention the LGPLv3 alternative" framing (as written) the right level of opinionation for the authored skill, or should it pick one Rails admin default outright?

## Target file(s) + estimated length
- skills/project-incubation/references/preferred-libraries/business-applications.md — est. 260 lines
